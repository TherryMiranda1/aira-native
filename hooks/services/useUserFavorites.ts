...
export const useUserFavorites = (contentType?: ContentType) => {
  ...
  const loadFavorites = useCallback(async () => {
    ...
    try {
      ...
      if (contentType) {
        userFavorites = await userFavoritesService.getUserFavoritesByType(user.id, contentType);
      } else {
  ...
  const isFavorite = useCallback((contentId: string, contentTypeToCheck?: ContentType): boolean => {
    ...
    return favorites.some(fav => 
      ...
      fav.contentType === typeToCheck
    );
  }, [favorites, user?.id, contentType]);

  // Obtener favorito específico
  const getFavorite = useCallback((contentId: string, contentTypeToCheck?: ContentType): UserFavorite | null => {
  ...
  const addToFavorites = useCallback(async (
    ...
    try {
      ...
      const newFavorite = await userFavoritesService.addToFavorites(favoriteData);
  ...
  const toggleFavorite = useCallback(async (
    ...
    if (isCurrentlyFavorite) {
      ...
      const result = await addToFavorites(contentId, contentTypeToToggle, notes);
      return result !== null;
    }
  }, [isFavorite, addToFavorites, removeFromFavorites]);

  // Incrementar contador de acceso
  const incrementAccessCount = useCallback(async (contentId: string, contentTypeToIncrement?: ContentType): Promise<void> => {
    if (!user?.id) return;
    ...
    try {
      const updatedFavorite = await userFavoritesService.incrementAccessCount(favorite.id);
  ...
  // Buscar en favoritos
  const searchFavorites = useCallback(async (query: string): Promise<UserFavorite[]> => {
    if (!user?.id || !query.trim()) return [];
    
    try {
      return await userFavoritesService.searchUserFavorites(user.id, query.trim());
    } catch (err) {
  ...
  const getFavoritesByCategory = useCallback((category: string): UserFavorite[] => {
    return favorites.filter(fav => fav.category === category);
  }, [favorites]);

  // Obtener favoritos más accedidos
  const getMostAccessedFavorites = useCallback((limit: number = 5): UserFavorite[] => {
    return favorites
  ...
  const getRecentFavorites = useCallback((limit: number = 10): UserFavorite[] => {
  ...
  const isToggling = useCallback((contentId: string): boolean => {
    return toggleLoading === contentId;
  }, [toggleLoading]);

  // Obtener conteo por tipo de contenido
  const getCountByType = useCallback((type: ContentType): number => {
