...
import { phraseService, Phrase, PhraseFilters } from '@/services/api/phrase.service'
...
interface UsePhrasesReturn {
  ...
  fetchPhrasesByMoment: (momento: 'manana' | 'dia' | 'tarde' | 'noche' | 'cualquier-momento') => Promise<void>
...
export function usePhrases(options: UsePhrasesOptions = {}): UsePhrasesReturn {
  ...
  const fetchPhrases = useCallback(async (customFilters?: PhraseFilters) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al cargar las frases')
      setPhrases([])
    } finally {
      setLoading(false)
    }
  ...
  const fetchPhrasesByCategory = useCallback(async (categoria: string) => {
    ...
    try {
      const categoryPhrases = await phraseService.getPhrasesByCategory(categoria)
      setPhrases(categoryPhrases)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error al cargar las frases de la categoría')
      setPhrases([])
    } finally {
      setLoading(false)
    }
  ...
  const fetchPhrasesByMoment = useCallback(async (momento: 'manana' | 'dia' | 'tarde' | 'noche' | 'cualquier-momento') => {
    setLoading(true)
    setError(null)
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al cargar las frases por momento')
      setPhrases([])
    } finally {
      setLoading(false)
    }
  ...
  const fetchPhrasesByEmotion = useCallback(async (estado: string) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al cargar las frases por estado emocional')
      setPhrases([])
    } finally {
      setLoading(false)
    }
  ...
  const fetchPopularPhrases = useCallback(async (limit = 10) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al cargar las frases populares')
      setPhrases([])
    } finally {
      setLoading(false)
    }
  ...
  const fetchRandomPhrases = useCallback(async (limit = 5) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al cargar frases aleatorias')
      setPhrases([])
    } finally {
      setLoading(false)
    }
  ...
  const getPhrasesByCategory = useCallback((categoria: string): Phrase[] => {
  ...
  }, [autoFetch, fetchPhrases, fetchCategories])
