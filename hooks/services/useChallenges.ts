...
import { challengeService, Challenge, ChallengeFilters } from '@/services/api/challenge.service'
...
export function useChallenges(options: UseChallengesOptions = {}): UseChallengesReturn {
  ...
  const [selectedCategory, setSelectedCategory] = useState<string | null>(filters.categoria || null)
  ...
  const fetchChallenges = useCallback(async (customFilters?: ChallengeFilters) => {
    setLoading(true)
    setError(null)
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al cargar los mini retos')
      setChallenges([])
    } finally {
      setLoading(false)
    }
  ...
  const fetchChallengesByCategory = useCallback(async (categoria: string) => {
    ...
    try {
      const categoryChangelles = await challengeService.getChallengesByCategory(categoria)
      setChallenges(categoryChangelles)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error al cargar los mini retos de la categoría')
      setChallenges([])
    } finally {
      setLoading(false)
    }
  ...
  const fetchPopularChallenges = useCallback(async (limit = 10) => {
    ...
    try {
      const popularChallenges = await challengeService.getPopularChallenges(limit)
      setChallenges(popularChallenges)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error al cargar los mini retos populares')
      setChallenges([])
    } finally {
      setLoading(false)
    }
  ...
  const fetchRandomChallenges = useCallback(async (limit = 5) => {
    ...
    try {
      const randomChallenges = await challengeService.getRandomChallenges(limit)
      setChallenges(randomChallenges)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error al cargar mini retos aleatorios')
      setChallenges([])
    } finally {
      setLoading(false)
    }
  ...
  const searchChallenges = useCallback(async (query: string) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al buscar mini retos')
      setChallenges([])
    } finally {
      setLoading(false)
    }
  ...
  const fetchCategories = useCallback(async () => {
    try {
      const availableCategories = await challengeService.getAvailableCategories()
  ...
  const setCategory = useCallback((categoria: string | null) => {
    setSelectedCategory(categoria)
    if (categoria) {
  ...
  const getChallengesByCategory = useCallback((categoria: string): Challenge[] => {
    return challenges.filter(challenge => challenge.categoria === categoria)
  }, [challenges])
